...
def insert():
    ...
    value = (id, name, score, sex, QQ)
    insert_sql = '''INSERT INTO list(id,name, score,sex,QQ) values (%s,%s,%s,%s,%s)'''
    cursor = get_cursor()  # 打开数据库
...
def update():
    ...
    sex = request.form.get("sex")
    QQ = request.form.get("QQ")
    value = ( name, age, score, sex,QQ)
    cursor = get_cursor()  # 打开数据库
    cursor.execute("update list set name = %s,score = %s,sex =%s,QQ = %s  where id = %s", value)  # 删除某一行值
    msg="编辑成功！"
    return msg
